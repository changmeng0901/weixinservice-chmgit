...
function InitVideoData(_data){
    ...
    if( _data.hasDevice == false){
            ...
            $('#tunnelName2').html(tunnelName);
            $('#tunnelName3').html(tunnelName);
            // 作物
			...
			if( _data.devices=='' || _data.devices==[] ||　_data.devices=='undefined' ){
				...
				FarmSwiper = new Swiper('#swiper_live',{
						...
						onSlideChangeEnd : function(){
							...
							CreatVideo2(FarmSwiper.activeIndex ,_data.devices[FarmSwiper.activeIndex].webcam_url,'../../images/MemberFarm/PlantDetail_temp1.jpg');
						}
					});
			}
                ...
                $(elem).click(function(){
                    ...
                    if( $('.monitor_tabhd .items').eq(0).attr('onoff') == 'true' ){
						...
						if( _data.images=='' || _data.images==[] ||　_data.images==undefined ){
							...
							$('#swiper_takePhotos .swiper-slide').eq(0).find('img').attr('src',$('#swiper_takePhotos .swiper-slide').eq(0).find('img').attr('data-src'));
							$('#takeNum').html( TakeSwiper.activeIndex+1 );
							$('#takeTotal').html( swiperTakeLen );
